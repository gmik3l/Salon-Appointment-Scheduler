...
MAIN_MENU() {
  ...
  # when there is no service available
  if [[ -z $SERVICES ]]
  then
    echo "Sorry, we don't have any service right now"
  # cdisplay services (formated)
  else
  ...
  # if selection isn't number
    if [[ ! $SERVICE_ID_SELECTED =~ ^[0-9]+$ ]]
    then
    # send to main menu with message
      ...
      # if selection is number but not valid
      if [[ -z $VALID_SERVICE ]]
      then
      # send to main menu with message
